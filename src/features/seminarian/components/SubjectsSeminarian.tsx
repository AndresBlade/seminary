...
const SubjectsSeminarian = ({notesByPeriod,setScoreDetailsBySubject,setShowModal}:SubjectsSeminarianProps) => {
    return (
        <div className={SeminarianCSS.subjectListSeminarian}>
            ...
            <div className={SeminarianCSS.stagePeriod}>
                <p>{notesByPeriod?.map(enrollment=>
                    enrollment.enrollment.map(date=>
                        date.start_date.slice(0,4) + '-' +
                        date.end_date.slice(0,4)
                    )
                )}</p>
            </div>
            {notesByPeriod === null ? <p>No hay materias para mostrar</p>:
                notesByPeriod.map(enrollment=>
                    enrollment.enrollment.map(subject=>
                        <div className={SeminarianCSS.subject} key={subject.subject_id}>
                            <p className={SeminarianCSS.subjectName}>{subject.subject_name}</p>
                            <div className={SeminarianCSS.scoreSubject}>
                                <p>{subject.subject_total_score_out_of_graded_score ? (parseFloat(subject.subject_total_score_out_of_graded_scored_10_scale)*2).toFixed(2) : 0}</p>
                                <button onClick={(e)=>{
                                    e.preventDefault()
                                    setScoreDetailsBySubject(subject.subject_id)
                                    setShowModal(true)
                                }}>
