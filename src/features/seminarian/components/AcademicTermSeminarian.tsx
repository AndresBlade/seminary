...
const AcademicTermSeminarian = ({dataAcademicterm,setPeriod}:AcademicTermSeminarianProps) => {
    return (
        <div className={SeminarianCSS.academicTermSeminarian}>
            {dataAcademicterm?.enrollment.map(enrollment=>
                enrollment.academic_term.map(period=>
                    <button key={period.academic_term_id} className={SeminarianCSS.buttonPeriod} onClick={(e)=>{
                        e.preventDefault()
