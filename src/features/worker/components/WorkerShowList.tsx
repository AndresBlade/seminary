...
export const WorkerShowList = () => {
	...
	const [dataWorkerToList, setDataWorkerToList] = useState<
		dataGetWorker[] | null
	>(null);
	...
	useEffect(() => {
		if (!user?.token) return;
		if (workerDelete !== '') {
			DeleteWorker({ id: workerDelete, token: user?.token })
				.then(response => {
					if (response.ok) {
	...
	}, [workerDelete, user?.token]);

	useEffect(() => {
		if (!user?.token) return;
		GetWorkerFound({ id: workerToFind, token: user?.token })
			.then(response => {
				setDataWorkerToList(response);
			})
			.catch(error => {
